...
class TestProductModel(unittest.TestCase):
    ...
    def test_deserialize_with_invalid_available_type(self):
        """It should raise DataValidationError with the correct error message for 'available'"""
        data = {
            "name": "Fedora",
            "description": "A red hat",
            "price": "12.50",
            "available": "invalid_type",
            "category": "CLOTHS"
        }
        product = Product()
        with self.assertRaises(DataValidationError) as cm_pr:
    ...
    def test_find_by_price(self):
        ...
        products = ProductFactory.create_batch(5)
        price_to_find = Decimal("12.50")
        products[0].price = price_to_find
        products[0].update()
        ...
        count = len([product for product in products if product.price == price_to_find])
        self.assertEqual(found.count(), count)

        for product in found:
            self.assertEqual(product.price, price_to_find)
